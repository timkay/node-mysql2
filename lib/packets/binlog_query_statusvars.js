...
module.exports = function parseStatusVars() {
  return {};
}
...
//     key = buffer[offset++];
//     const remain = buffer.length - offset;
//     switch (key) {
//       case keys.FLAGS2:
//         result.flags = 0;
//         if (remain >= 4) result.flags = buffer.readUInt32LE(offset);
//         offset += 4;
//         break;
//       case keys.SQL_MODE:
//         // value is 8 bytes, but all dcumented flags are in first 4 bytes
//         result.sqlMode = 0;
//         if (remain >= 4) result.sqlMode = buffer.readUInt32LE(offset);
//         offset += 8;
//         break;
//       case keys.CATALOG:
...
//         result.schemaCharset = 0;
//         if (remain >= 2) result.schemaCharset = buffer.readUInt16LE(offset);
//         offset += 2;
//         break;
